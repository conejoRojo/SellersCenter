...
1. **Dashboard de Comercio (Sellers)**
   ...
   *   **URL (Túnel Público):** `https://pull-powerpoint-long-leads.trycloudflare.com`
...
2. **Consola de Administración (Aper Admin)**
   ...
   *   **URL (Túnel Público):** `https://camcorder-bat-nationally-lakes.trycloudflare.com`
...
1. Envíe un payload JSON simulando una actualización de producto desde PrestaShop al endpoint público del backend (`https://nov-gerald-copyrighted-incoming.trycloudflare.com/api/v1/sync/ingesta/`). *(Nota: Cloudflare no requiere contraseña de tunnel. Si intentas visitar la url base desde el navegador, arrojará "Page not found (404)" ya que es una API sin interfaz gráfica en la raíz).*
